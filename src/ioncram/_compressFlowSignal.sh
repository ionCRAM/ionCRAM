...
samtools view -h - | tee $samIn1  | grep -oP "ZM:B:s,[^\t]*"|sed -e 's/ZM:B:s,//' | ${binFolder}writer_varLenDiffSep   $fs $fs2 $binning &
...
if [ $compression == 'xz' ]
then
    fsOut=$Out_prefix.fs.binning$binning.xz
...
then
    fsOut=$Out_prefix.fs.binning$binning.gz
