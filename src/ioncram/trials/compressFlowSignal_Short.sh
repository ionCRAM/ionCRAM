...
mkfifo $header $sam1 $fs 

./icram view -H $Bam >$header &

cat $fs | gzip -9 -c > $Out_prefix.fs.gz &
#cat $fs2 | zstd --ultra -22 -c  > $Out_prefix.fs2.gz &
#cat $fs3 | zstd --ultra -22 -c  > $Out_prefix.fs3.gz &
./icram view $Bam |tee $sam1  | grep -oP "ZM:B:s,[^\t]*"|sed -e 's/ZM:B:s,//' | ./writer_short   $fs      & 
...
tar -cf $Out_tar $Out_prefix.cram  $Out_prefix.fs.gz  
rm $Out_prefix.cram  $Out_prefix.fs.gz    tmp.$r.* 
