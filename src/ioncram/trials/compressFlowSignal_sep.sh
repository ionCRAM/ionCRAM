...
mkfifo $header $sam1 $fs $fs2 

./icram view -H $Bam >$header &

cat $fs | xz -e -9 -c > $Out_prefix.fs.gz &
cat $fs2 | xz -e  -9  -c  > $Out_prefix.fs2.gz &

./icram view $Bam |tee $sam1  | grep -oP "ZM:B:s,[^\t]*"|sed -e 's/ZM:B:s,//' | ./writer_varLenDiffSep   $fs $fs2 $     & 
...
tar -cf $Out_tar $Out_prefix.cram  $Out_prefix.fs.gz   $Out_prefix.fs2.gz     
rm $Out_prefix.cram  $Out_prefix.fs.gz    tmp.$r.* $Out_prefix.fs2.gz   
